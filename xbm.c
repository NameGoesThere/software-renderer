...
void drawText(const char *text, int len, int x, int y, Color color) {
	for (int i = 0; i < len; ++i) {
		const char *charData =
			ascii_8x13_font[(unsigned char)text[i] - FONT_ASCII_OFFSET];

		for (int j = 0; j < FONT_HEIGHT; ++j) {
			for (int k = 0; k < FONT_WIDTH; ++k) {
				if ((charData[j] >> k) & 1) {
					drawPixel(x - k + (i + 1) * FONT_WIDTH, y - j + FONT_HEIGHT,
							  color);
				}
			}
		}
...
const char ascii_8x13_font[94][13] = {{0x00, 0x00, 0x18, 0x18, 0x00, 0x00, 0x18,
